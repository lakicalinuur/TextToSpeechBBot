...
def generate_tts_filename(user_id, text, voice_label):
    clean_text = "".join([c for c in text[:10] if c.isalnum()])
    file_name = f"{BOT_NAME}_{user_id}_{clean_text}_{voice_label.split()[0]}.mp3"
...
def handle_text(message):
    ...
    text_to_process = message.text.replace(".", ",")
    voice_name = CURRENT_VOICE_NAME
    filename = generate_tts_filename(user_id, text_to_process, CURRENT_VOICE_LABEL)
    ...
    async def make_tts():
        ...
        tts = edge_tts.Communicate(text_to_process, voice_name, rate=rate, pitch=pitch)
